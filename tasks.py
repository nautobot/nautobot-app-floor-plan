...
def pylint(context, target=None, recursive=False):
    ...
    if recursive:
        command += " --recursive=y"
    command += f" {' '.join(target) if target else 'nautobot_floor_plan'}"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    app_dir = Path(__file__).absolute().parent / Path("nautobot_floor_plan")
    migrations_dir = app_dir / Path("migrations")
    migrations_target_module = "nautobot_floor_plan.migrations"
...
def generate_test_data(context, flush=False, database=None):
    """Generate test data in Nautobot for Nautobot Floor Plan."""
    command = "nautobot-server generate_nautobot_floor_plan_test_data"
    if database:
