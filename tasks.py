...
def docker_compose(context, command, **kwargs):
    ...
    compose_command = f'docker-compose --project-name {context.nautobot_floor_plan.project_name} --project-directory "{context.nautobot_floor_plan.compose_dir}"'
...
def docs(context):
    ...
    if is_truthy(context.nautobot_floor_plan.local):
        ...
        print("Only used when developing locally (i.e. context.nautobot_floor_plan.local=True)!")
...
def pylint(context):
    ...
    command = 'pylint --init-hook "import nautobot; nautobot.setup()" --rcfile pyproject.toml nautobot_floor_plan'
    run_command(context, command)
...
def unittest(context, keepdb=False, label="nautobot_floor_plan", failfast=False, buffer=True):
